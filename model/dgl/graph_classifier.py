...
class GraphClassifier(nn.Module):
    def __init__(self, params, relation2id, ent2rels):  # in_dim, h_dim, rel_emb_dim, out_dim, num_rels, num_bases):
        ...
        # self.rel_emb = nn.Embedding(self.params.num_rels + 1, self.params.rel_emb_dim, sparse=False, padding_idx=self.params.num_rels)
        # Set the dim of both node and relation embedding as inp_dim.
        ...
        # self.ent_sem_dim = self.params.sem_dim
        # self.w_rel2ent = nn.Linear(self.params.rel_emb_dim, self.ent_sem_dim)
        ...
        # self.params.emb_dim = self.params.emb_dim + self.ent_sem_dim
    ...
    def nei_rel_path(self, g, rel_labels, r_emb_out):
        ...
        for (head_rels, r_t, tail_rels) in zip(heads_rels, rel_labels, tails_rels):
            ...
            # Embeddings of all paths for certain target triple
            # paths = torch.LongTensor(paths) # [n_paths, 3]
            # path_embs = self.rel_emb(paths) # [n_paths, 3, inp_dim]
        # print("Extract NRP time: ", time.time() - t_s)
        ...
        # batch_paths_embs = self.rel_emb(batch_paths) # [B * n_paths, 3, inp_dim]
        ...
        # print("NRP RNN time: ", time.time() - t_s)
        ...
        # print("NRP AGG time: ", time.time() - t_s)
    ...
    def forward(self, data, is_return_emb=False, cor_graph=False):
        ...
        # print("init time:" , time.time() - ts)
        # self.init_target_ent(g, rel_labels)
        # self.init_ent_emd(g, rel_labels)
        # print('Initialize entity embedding time sused: ', time.time() - ts)
        
        # r: Embedding of relation
        r = self.rel_emb.weight.clone()
        g.ndata['h'], r_emb_out = self.gnn(g, r)
        
        # print("gnn time :" , time.time() - ts)
        # GRU layer for nodes
        ...
        # Represent subgraph by composing (h,r,t) in some way.
        if self.params.comp_hrt:
        ...
        if self.params.nei_rel_path:
            ...
            # print("nei rel path time:" , time.time() - ts)
        else:
            s_g = g_out
        ...
        if not is_return_emb:
            ...
            out_dim = self.params.num_gcn_layers * self.params.emb_dim
            ht_embs = self.comp_ht_emb(head_embs.view(-1, out_dim), tail_embs.view(-1, out_dim))

            s_G = s_g.mean(0)
            return output, ht_embs, s_G, s_g
